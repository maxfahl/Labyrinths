...
function ControlsPanel({ options, setOptions }: ControlsPanelProps) {
  return (
    <Card className="w-full h-full flex flex-col bg-white text-gray-900 border shadow rounded-2xl">
      ...
      <CardContent
        className="flex-1 flex flex-col gap-6 overflow-auto max-h-[80vh] scrollbar-none"
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          <div className="flex flex-row gap-4 items-center">
            <div className="flex flex-col items-center gap-1">
              <Label htmlFor="line-color" className="">
                Line Color
              </Label>
              <Input
                id="line-color"
            ...
            <div className="flex flex-col items-center gap-1">
              <Label htmlFor="bg-color" className="">
                Background Color
              </Label>
              <Input
                id="bg-color"
        ...
        <div
          ...
          role="group"
          aria-labelledby="wall-thickness-label"
        >
          <Label
            id="wall-thickness-label"
            htmlFor="wall-thickness"
            className=""
          >
            Wall Thickness: {options.wallThickness}px
          </Label>
          <Slider
            id="wall-thickness"
            min={1}
            ...
            onValueChange={([val]: [number]) =>
              setOptions({ ...options, wallThickness: val })
            }
        ...
        {/* SVG Padding Control */}
        <div className="flex flex-col gap-2">
          <Label htmlFor="svg-padding" className="">
            Maze Padding (px)
          </Label>
          <Input
            id="svg-padding"
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              setOptions({ ...options, svgPadding: Number(e.target.value) })
            }
        ...
        <div className="flex flex-col md:flex-row gap-2">
          <div className="flex flex-col gap-2 flex-1">
            <Label htmlFor="start-position" className="">
              Start Position
            </Label>
            <select
              id="start-position"
              ...
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                setOptions({ ...options, startPosition: e.target.value })
              }
          ...
          <div className="flex flex-col gap-2 flex-1">
            <Label htmlFor="end-position" className="">
              End Position
            </Label>
            <select
              id="end-position"
              ...
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                setOptions({ ...options, endPosition: e.target.value })
              }
              className="border rounded px-2 py-1"
            >
              {START_END_POSITIONS.map((opt) => (
        ...
        >
          <Label id="seed-label" htmlFor="seed" className="">
            Seed: {options.seed}
          </Label>
          <Slider
            id="seed"
            ...
            onValueChange={([val]: [number]) =>
              setOptions({ ...options, seed: String(val) })
            }
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              setOptions({ ...options, showSolution: e.target.checked })
            }
            aria-label="Show Solution"
            className="accent-primary w-5 h-5"
          />
          <Label htmlFor="show-solution" className="">
            Show Solution
          </Label>
        </div>
